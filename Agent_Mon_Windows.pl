import subprocess
rcCnt = 1
p1 = subprocess.Popen(['/opt/OV/bin/OpC/utils/opcnode','-list_nodes','group_name=windows','net_type=NETWORK_IP'], stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
for i_u in out.split("Name         = "):
        fobj= open("/opt/Mon_Scripts/Windows_Failure_Rpt.log",'w')
        ...
        rcCnt +=1
rptHTML+="</table>"
fobj= open("/opt/Mon_Scripts/Windows_Failure_Rpt.html",'w')
fobj.write(rptHTML)
fobj.close()
fail_p = subprocess.Popen(['sh','/opt/Mon_Scripts/Windows_Mail.sh'], stdout=subprocess.PIPE,stderr=subprocess.PIPE)
